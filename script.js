...
var teksSekarang = 1;var ftom=0;var pesanwhatsapp = "";const body = document.querySelector("body"); audio = new Audio('' + linkmp3.src); 
    var swiper = new Swiper(".mySwiper", {
      ...
      slideChange: function () {
        ...
        setTimeout(function(){
          ...
          if(teksSekarang == 10){
            //pesanwhatsapp = "1%29%20" + teks2.innerText +"%0A%20%20%20%20%20%2AJawab%3A%2A%20" + iteks2.value + "%0A2%29%20" + teks3.innerText +"%0A%20%20%20%20%20%2AJawab%3A%2A%20" + iteks3.value + "%0A3%29%20" + teks4.innerText +"%0A%20%20%20%20%20%2AJawab%3A%2A%20" + iteks4.value + "%0A4%29%20" + teks5.innerText +"%0A%20%20%20%20%20%2AJawab%3A%2A%20" + iteks5.value + "%0A5%29%20" + teks6.innerText +"%0A%20%20%20%20%20%2AJawab%3A%2A%20" + iteks6.value + "%0A6%29%20" + teks7.innerText +"%0A%20%20%20%20%20%2AJawab%3A%2A%20" + iteks7.value + "%0A7%29%20" + teks8.innerText +"%0A%20%20%20%20%20%2AJawab%3A%2A%20" + iteks8.value + "%0A8%29%20" + teks9.innerText +"%0A%20%20%20%20%20%2AJawab%3A%2A%20" + iteks9.value + "";
            //pesanwhatsapp="Baik%3A%20100%25%F0%9F%AB%A2%0ARandom%3A%20100%25%F0%9F%A4%AA%0AGaje%3A%20100%25%F0%9F%98%9D%0ANakal%3A%20100%25%F0%9F%A4%AC%0APerhatian%3A%20100%25%F0%9F%98%8B%0AAku%20Sayang%20Kamu%3A%20100%25%F0%9F%98%BB%F0%9F%92%90%0APanggilan%20buat%20Kamu%3A%20ayang%F0%9F%98%8B%F0%9F%A4%8C%F0%9F%8F%BB%0A3%20Kata%20buat%20Kamu%3A%20Baik%2C%20Ganteng%2C%20Lucuu%20%F0%9F%98%86%F0%9F%92%97";
			pesanwhatsapp="Baik%3A%20" + rangeValue2.textContent + "25%F0%9F%AB%A2%0ARandom%3A%20" + rangeValue3.textContent + "25%F0%9F%A4%AA%0AGaje%3A%20" + rangeValue4.textContent + "25%F0%9F%98%9D%0ANakal%3A%20" + rangeValue5.textContent + "25%F0%9F%A4%AC%0APerhatian%3A%20" + rangeValue6.textContent + "25%F0%9F%98%8B%0AAku%20Sayang%20Kamu%3A%20" + rangeValue7.textContent + "25%F0%9F%98%BB%F0%9F%92%90%0APanggilan%20buat%20Kamu%3A%20" + iteks8.value + "%F0%9F%98%8B%F0%9F%A4%8C%F0%9F%8F%BB%0A3%20Kata%20buat%20Kamu%3A%20" + iteks9.value;
            katanimasi()
          }
        }, 50);
      },
    ...
    // Radio
    document.getElementById('iteks2').addEventListener('input', function() {
	    const val = this.value;
	    const percent = (val - this.min) / (this.max - this.min) * 100;
	    const thumbPos = (val / 100) * (this.offsetWidth - 18);
	    const label = document.getElementById('rangeValue2');
	    
	    label.textContent = val + '%';
	    label.style.left = (thumbPos + 9) + 'px';
	
	    this.style.setProperty('--progress', percent + '%');
	});
	
	document.getElementById('iteks3').addEventListener('input', function() {
	    const val = this.value;
	    const percent = (val - this.min) / (this.max - this.min) * 100;
	    const thumbPos = (val / 100) * (this.offsetWidth - 18);
	    const label = document.getElementById('rangeValue3');
	...
	document.getElementById('iteks4').addEventListener('input', function() {
	    const val = this.value;
	    const percent = (val - this.min) / (this.max - this.min) * 100;
	    const thumbPos = (val / 100) * (this.offsetWidth - 18);
	    const label = document.getElementById('rangeValue4');
	...
	document.getElementById('iteks5').addEventListener('input', function() {
	    const val = this.value;
	    const percent = (val - this.min) / (this.max - this.min) * 100;
	    const thumbPos = (val / 100) * (this.offsetWidth - 18);
	    const label = document.getElementById('rangeValue5');
	...
	document.getElementById('iteks6').addEventListener('input', function() {
	    const val = this.value;
	    const percent = (val - this.min) / (this.max - this.min) * 100;
	    const thumbPos = (val / 100) * (this.offsetWidth - 18);
	    const label = document.getElementById('rangeValue6');
	...
	document.getElementById('iteks7').addEventListener('input', function() {
	    const val = this.value;
	    const percent = (val - this.min) / (this.max - this.min) * 100;
	    const thumbPos = (val / 100) * (this.offsetWidth - 18);
	    const label = document.getElementById('rangeValue7');
    ...
    varteks = teks10.innerHTML; teks10.innerHTML = "";
    function katanimasi(){
	  new TypeIt("#teks10", {
      strings: ["" + varteks], startDelay: 250, speed: 25, cursor: true,
      afterComplete: function(){
      	clearInterval(scrollInterval);
      	teks10.innerHTML = varteks;
...
//function fungsimau(){Tombol.style="opacity:0";teks10.style = "font-size:17px;text-align:left;margin:0 55px 0 40px;opacity:0;transition:all .5s ease";setTimeout(function(){varteksditerima = teksditerima.innerHTML;teks10.style = "font-size:17px;text-align:left;margin:0 55px 0 40px;opacity:1;transform:scale(1);transition:all .6s ease";Tombol.style="display:none";teks10.innerHTML = "";katanimasi2();},300);}
...
function katanimasi2(){
      ...
      afterComplete: function(){
      	clearInterval(scrollInterval);
      	teks10.innerHTML = varteksditerima;
      	setInterval(berjatuhan, 200);
      },}).go();
}
